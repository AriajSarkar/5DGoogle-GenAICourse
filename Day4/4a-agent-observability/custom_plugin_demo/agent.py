"""
Research Paper Finder Agent with Custom CountInvocationPlugin

This agent demonstrates custom plugin creation for specialized observability needs.
Shows how to build plugins with callbacks for tracking custom metrics.

Learning Objectives:
- Create custom plugins by extending BasePlugin
- Implement before_agent_callback and before_model_callback
- Track custom metrics (agent invocations, LLM requests)
- Understand when to use custom plugins vs LoggingPlugin

Run with:
    adk run custom_plugin_demo/
"""
...
# Root agent (custom plugin added via runner)
root_agent = LlmAgent(
